...
let experimentRequest=0, experimentFingerprint='';
async function experiments(quiet=false){const request=++experimentRequest,d=await api('/api/metrics'),box=$('#experiment-content');if(request!==experimentRequest)return;const fingerprint=JSON.stringify(d);if(quiet&&fingerprint===experimentFingerprint)return;experimentFingerprint=fingerprint;box.replaceChildren();
const refreshButton=el('button','text-button','刷新训练结果');refreshButton.addEventListener('click',()=>experiments().catch(error));box.append(refreshButton);
const run=d.training_run;
box.append(el('p','footnote',run?`训练批次：${run.run_number} · ${run.run_id}；开始：${new Date(run.started_at).toLocaleString()}；${run.status==='completed'?'已完成':'训练中或未完成'} ${run.completed_epochs} / ${run.epochs} 轮${run.finished_at?'；完成：'+new Date(run.finished_at).toLocaleString():''}`:`当前已有训练：${d.history.length} 轮；此版本未记录训练批次与时间。`));
box.append(el('p','muted','每 10 秒检查最新训练记录。新一轮训练会覆盖上一批记录；相同数据和随机种子可能得到相同指标。'));
if(!d.available){box.append(el('p','',run?'本次训练尚未完成，暂不展示旧模型评测。':'训练结果尚未生成，请运行项目训练命令。'),trainingChart(d.history));return;}const m=d.metrics;box.append(el('p','footnote','当前模型：'+d.model_name+'。'+m.limitation));if(d.model_name!=='models_manual')box.append(el('p','error','独立实验模型：请先查看网页及改写集表现，不能仅凭模板高分替换默认模型。'));const row=el('div','metric-row');[['实体严格 F1',m.ner.f1],['关系宏平均 F1',m.relation.macro_f1],['关系准确率',m.relation.accuracy]].forEach(([name,value])=>{const c=el('div','white-card');c.append(el('p','',name),el('strong','',(value*100).toFixed(1)+'%'));row.append(c);});box.append(row);const extra=el('section','white-card');extra.append(el('h2','','模板外表现与使用范围'),el('p','','下面是纯神经模型保存的评测，不代表增强抽取效果。分数包含实体识别、关系抽取与阈值过滤，不能与上方使用金标准实体的关系指标混用。'));
  const extraRows=[];
  ...
  box.append(extra);
  const config=el('section','white-card');config.append(el('h2','', '实验设置'),el('p','',`训练 / 验证 / 测试：${m.dataset.train} / ${m.dataset.val} / ${m.dataset.test} 句。随机种子 ${m.seed}，最佳轮次 ${m.best_epoch}，用时 ${m.seconds.toFixed(1)} 秒。`),el('p','', 'NER 按实体边界与类型严格匹配；关系分类使用金标准实体位置，分数不等于端到端三元组准确率。'));box.append(config);box.append(trainingChart(d.history,m.best_epoch));const matrix=el('section','white-card');matrix.append(el('h2','','关系混淆矩阵'),el('p','muted','行是真实标签，列是预测标签。'),table(['真实 / 预测',...m.relation.label_order],m.relation.confusion_matrix.map((r,i)=>[m.relation.label_order[i],...r])));box.append(matrix);const errors=el('section','white-card');errors.append(el('h2','','错误案例'),table(['原句','真实关系','预测关系'],m.errors.slice(0,10).map(e=>[e.text,e.gold_relation,e.predicted_relation])));box.append(errors);}

function trainingChart(history,bestEpoch){
  const card=el('section','white-card');card.append(el('h2','','验证集 F1 随训练轮次变化'));
  if(!history.length){card.append(el('p','muted','等待本次训练完成第 1 轮验证。'));return card;}
  const width=Math.max(900,history.length*78+100),height=300,left=55,right=width-40;
  const x=i=>left+i*(right-left)/Math.max(1,history.length-1),y=v=>220-v*160;
  const scroll=el('div','table-wrap'),s=svgEl('svg',{viewBox:`0 0 ${width} ${height}`,width,height,role:'img','aria-label':'逐轮验证集 F1；横轴训练轮次，纵轴 F1（0 至 1）'});
  [0,.5,1].forEach(v=>s.append(svgEl('line',{x1:left,y1:y(v),x2:right,y2:y(v),stroke:'#e2e8de'}),svgEl('text',{x:15,y:y(v)+4,'font-size':12},String(v))));
  history.forEach((r,i)=>s.append(svgEl('text',{x:x(i),y:245,'font-size':12,'text-anchor':'middle'},String(r.epoch))));
  s.append(svgEl('text',{x:width/2,y:280,'font-size':14,'text-anchor':'middle'},'训练轮次（Epoch）'),svgEl('text',{x:15,y:25,'font-size':14},'F1'));
  [['val_ner_f1','#367043','实体 F1',-12],['val_relation_macro_f1','#286f9b','关系 F1',22]].forEach(([key,color,name,offset])=>{
    s.append(svgEl('polyline',{points:history.map((r,i)=>`${x(i)},${y(r[key])}`).join(' '),fill:'none',stroke:color,'stroke-width':2.5}));
    history.forEach((r,i)=>{const dot=svgEl('circle',{cx:x(i),cy:y(r[key]),r:4,fill:color});dot.append(svgEl('title',{},`第 ${r.epoch} 轮 ${name}：${r[key]}`));s.append(dot,svgEl('text',{x:x(i),y:y(r[key])+offset,fill:color,'font-size':11,'text-anchor':'middle'},r[key].toFixed(6)));});
  });
  scroll.append(s);card.append(scroll,el('p','muted',`绿色：实体严格 F1；蓝色：关系宏平均 F1。共 ${history.length} 轮${bestEpoch?'；最佳轮次：第 '+bestEpoch+' 轮':''}。图中保留 6 位小数，下表保留记录中的原始数值。`),table(['训练轮次','实体验证 F1','关系验证宏 F1','实体损失','关系损失','选模'],history.map(r=>[r.epoch,r.val_ner_f1,r.val_relation_macro_f1,r.ner_loss,r.relation_loss,r.epoch===bestEpoch?'最佳轮次':''])));return card;
}
setInterval(()=>{if(!$('#experiment').hidden)experiments(true).catch(error);},10000);
